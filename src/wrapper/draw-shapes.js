function drawPolygon(ctx, n, cx, cy, r) {
  ctx.beginPath()
  ctx.moveTo(cx + r, cy)
  for (let i = 1; i < n; i++) {
...
export function drawSymbol(ctx, shape, cx, cy, r) {
...
export function drawUnknown(ctx, cx, cy, r) {
  ctx.beginPath()
...
const RangeDrawers = {
  ...
  roundRectangle(ctx, x, y, width, height) {
  ...
  line(ctx, x, y, width, height, xPadding) {
    const cy = y + 0.5 * height
    drawLine(ctx, x + xPadding, cy, x + width - xPadding, cy)
  },

  bridge(ctx, x, y, width, height, xPadding, fragmentLength) {
    x += xPadding
    width -= 2 * xPadding
  ...
  discontinuosStart(ctx, x, y, width, height) {
    const qy = 0.2 * height
  ...
  discontinuosEnd(ctx, x, y, width, height) {
    const qy = 0.2 * height
    const qx = Math.min(qy, 0.5 * width)
    ctx.beginPath()
    ctx.moveTo(x, y)
  ...
  discontinuos(ctx, x, y, width, height) {
    const qy = 0.2 * height
    const qx = Math.min(qy, 0.5 * width)
    ctx.beginPath()
    ctx.moveTo(x, y)
  ...
  helix(ctx, x, y, width, height, xPadding) {
    x += xPadding
    width -= 2 * xPadding
  ...
  strand(ctx, x, y, width, height, xPadding) {
    x += xPadding
    width -= 2 * xPadding
