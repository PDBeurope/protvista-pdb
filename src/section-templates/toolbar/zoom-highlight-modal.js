...
import { validateResidue } from "../../helpers/layout/custom-tracks-validation";

const MODAL_TYPE = "zoom-highlight";
...
export default function PDBePvZoomHighlightModal(ctx) {
  const sequenceLength =
    Number(ctx.viewerData?.length ?? ctx.sequenceLength) || 0;

  const closeDialog = () => {
    ctx.layoutHelper.hideZoomHighlightDialog();
  };
  ...
  const validateInput = (input, allowEmpty = false) => {
    ...
    const { isValid, messages } = validateResidue(
      {
        start: 1,
        end: sequenceLength,
      },
    ...
    setError(input, isValid ? "" : messages[0]);

    return isValid;
  };

  const zoomOrHighlightTrack = (event) => {
    const protvistaPdbNavigator = ctx.querySelector("protvista-pdb-navigation");

    if (!protvistaPdbNavigator) return;

    if (event.type === "highlight") {
      protvistaPdbNavigator.dispatchEvent(
  ...
  const submit = (type) => {
    const modal = ctx.querySelector(".customTrackModalContainer.zoomHighlight");

    const startInput = modal?.querySelector(".zoomHighlightStart");
    const endInput = modal?.querySelector(".zoomHighlightEnd");

    const validStart = validateInput(startInput);
    const validEnd = validateInput(endInput, true);

    if (!validStart || !validEnd) return;

    const start = Number(startInput.value);
    const end = endInput.value.trim() === "" ? start : Number(endInput.value);

    if (type === "highlight") {
      zoomOrHighlightTrack({
        type,
        start,
        end
      });
    ...
    ctx.dispatchEvent(
      new CustomEvent("protvista-pdb-zoom-highlight-region", {
        bubbles: true,
        composed: true,
        detail: {
          type,
  ...
  return html`
    <div class="customTrackModalBackdrop">
      <section
        class="zoomCustomTrack"
        role="dialog"
        aria-modal="true"
        aria-labelledby="zoom-highlight-title"
        @keydown=${(event) => {
          if (event.key === "Escape") {
            closeDialog();
          }
      ...
      >
        <div class="customTrackRowGroup">
          <header class="customTrackDialogHeader">
            <h3 id="zoom-highlight-title" class="customTrackDialogTitle">
              ${ctx.forLigands
                ? "Find ligand atoms by index (CIF file sorting)"
                : "Find residue or range"}
            </h3>

            <button
              class="customTrackCloseButton"
              type="button"
              aria-label="Close zoom/highlight dialog"
              @click=${closeDialog}
            >
              ${closeIcon}
            </button>
          ...
          <div class="customTrackDialogBody">
            <div class="zoomHighlightFields">
              <div class="customTrackTextField">
                <label class="customTrackFieldLabel" for="zoom-highlight-start">
                  Start
                </label>

                <input
                  id="zoom-highlight-start"
                  class="customTrackInput zoomHighlightStart"
                  type="text"
                  name="start"
                  placeholder="1"
                  aria-invalid="false"
              ...
              <div class="customTrackTextField">
                <label class="customTrackFieldLabel" for="zoom-highlight-end">
                  End
                </label>

                <input
                  id="zoom-highlight-end"
                  class="customTrackInput zoomHighlightEnd"
                  type="text"
                  name="end"
                  placeholder=${String(sequenceLength)}
                  aria-invalid="false"
                  @input=${(event) => {
                    validateInput(event.currentTarget, true);
                  }}
                />
          ...
          <footer class="customTrackDialogFooter">
            <div class="customTrackFooterActions">
              <button
                class="customTrackSecondaryButton"
                type="button"
